...
class MctsTree:
    def __init__(self) -> None:
        ...
        self.RtSpMctsNode = GlbVar.MctsNode() # 0:Root, 1:Left tree, 2:Right tree
        self.RtSpMctsNode.node = GlbVar.Node(0.1, 0.1, 0.1)   # The coordinates of the start and target node should be given externally [important]
        self.RtSpMctsNode.TreeId = 1
        self.RtSpMctsNode.TreeLvl = 1

        self.RtTpMctsNode = GlbVar.MctsNode()
        self.RtTpMctsNode.node = GlbVar.Node(5, 5, 0.6)
        self.RtTpMctsNode.TreeId = 2
        self.RtTpMctsNode.TreeLvl = 1

        self.RootMctsNode = GlbVar.MctsNode()
        self.RootMctsNode.children.append(self.RtSpMctsNode)
        self.RootMctsNode.children.append(self.RtTpMctsNode)

        self.RtSpMctsNode.parent = self.RootMctsNode
        self.RtTpMctsNode.parent = self.RootMctsNode

        self.SpTreeSelectNode = self.RtSpMctsNode   # RS should connect SpTreeSelectNode and TpTreeSelectNode
        self.TpTreeSelectNode = self.RtTpMctsNode
...
# ----------------------- Select Node ----------------------
    ...
    def SelectNode(self, crnt_node):
        ...
        for child_node in crnt_node.children:
            ...
            exploration_term = np.sqrt(child_node.parent.n_visit) / (1 + child_node.n_visit)
            puct_value = exploitation_term + self.exploration_weight * child_node.Probs * exploration_term  # PUCT in AlphaZero
...
# ----------------------- Expand Node ----------------------
    ...
    def ExpandNode(self, crnt_node):
        ...
        for node in new_node_list:
            ...
            NewMctsNode.TreeId = NewMctsNode.parent.TreeId
            NewMctsNode.TreeLvl = NewMctsNode.parent.TreeLvl + 1
            DnnV, DnnP = [random.uniform(0, 100), random.uniform(0, 1)]    # Need delete [important]
            NewMctsNode.Value = DnnV    # Need update [important]
            NewMctsNode.Probs = DnnP    # Need update [important]
            ...
            NewMctsNode.ActCost = - abs(crnt_node.ActInfo[0] - NewMctsNode.ActInfo[0]) * Config.TreePara.SteerSpdCostCoff \
                                - abs(crnt_node.ActInfo[1] - NewMctsNode.ActInfo[1]) * Config.TreePara.GearShiftCostCoff
...
# ---------------------- Backpropagate ---------------------
    ...
    def BackpropagateType(self, node):
        ...
        if node.parent == None or node.type == 1:
            return
    ...
    def BackpropagateValue(self, node):
        ...
        if node.n_visit == 1:   # True value node
            node.Value = DrlUtil.LeafNodeValueJudge(node)
            node.Vdone = True
            return
        elif node.n_visit == 0: # Leaf node
            node.Value = 10 if node.type == 1 else 0    # Givn by Dnn may be a better choice
            node.Vdone = True
            return
        ...
        if all_child_Vdone:
            ...
            for child_node in node.children:
                itmdt_reward = 0
                probs = child_node.n_visit / (node.n_visit - 1)
                node_value = node_value + (itmdt_reward + self.gamma * child_node.Value) * probs
...
# ----------------------- Simulation -----------------------
    ...
    def Simulate(self):
        for cnt in range(self.expd_maxcnt):
            ...
            # 1. Select Node
            # 1.1 Explore and select until a leaf node is found, and store the selected leaf node into RtSpMctsNode or RtTpMctsNode
            while True:
                if DrlUtil.IsLeafNode(node):
                    SelectedLeafNode = node
                    
                    if SelectedLeafNode.TreeId == 1:
                        self.RtSpMctsNode = SelectedLeafNode
                    elif SelectedLeafNode.TreeId == 2:
                        self.RtTpMctsNode = SelectedLeafNode
                    break
            ...
            if SelectNodeInfo == 1:   # Case for normal leaf node
                PathFnd, _ = DrlUtil.CalValidRS(self.RtSpMctsNode.node, self.RtTpMctsNode.node)
                self.RtSpMctsNode.type = 4 if PathFnd else 1
                self.RtTpMctsNode.type = 4 if PathFnd else 1

                _ = self.BackpropagateType(self.RtSpMctsNode)
                _ = self.BackpropagateType(self.RtTpMctsNode)
            ...
            for ExpdNode in Expd_MctsNode_list:
                _ = self.BackpropagateType(ExpdNode)
...
# ---------------------- StoreTreeInfo ---------------------
    ...
    def StoreTreeInfo(self):
        ...
        self.TravslTreeInfo(self.RootMctsNode, state_list, act_probs_list, V_value_list)
...
# ---------------------- Visualization ---------------------
    def VisTree(self):
        ...
        time.sleep(1)   # Wait 1 second to prevent duplicate names
    ...
    def add_nodes(self, node, dot):
        ...
        label = f"({formatted_x}, {formatted_y}, {formatted_yaw_rad})\
            \nTreeId: {node.TreeId}, Value: {formatted_V}, Probs: {formatted_Probs}, n_visit: {node.n_visit}, Vdone: {node.Vdone}\
            \nType: {node.type}, ActInfo: {formatted_ActInfo}, ActCost: {node.ActCost}"  # Include ActCost in the label
