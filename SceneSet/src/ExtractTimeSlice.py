...
def ets(file, sample_mode, sample_nums):    
    ...
    for index in time_slice_list:
        ...
        SsmState = mdf_resample.loc[index, 'PFSM_SYSM_HMIOutOutputs_o_obv.PAS_State']
        Compute_State = mdf_resample.loc[index, 'PFSM_ZF_Trajectory_Compute_State_i_obv']
        ReplanType = mdf_resample.loc[index, 'SA_Replanning_Trigger_Type_NU_o_obv']
        ...
        OD_Number, OD_Confidence, OD_x, OD_y = DataUtil.get_object_information(mdf_resample, index)
        FSB_Number, FSB_Confidence, FSB_x, FSB_y = DataUtil.get_freespace_information(mdf_resample, index)

        new_data = pd.DataFrame({"FileName": [os.path.basename(file)],
                                "TimeStamp": [TimeStamp],
                                "PfsmState": [PfsmState],
                                "SsmState": [SsmState],
        ...
        log_info = DataUtil.gen_log_info(Mf4_duration_OK, PfsmState, SsmState, PrkMod, StartPose, TargetPose, ParkingSlot_type, OD_Number, FSB_Number)
